...
const Timeline_Bildung = ({ className }) => (
  <div className={className}>
    <h1>Bildungsweg</h1>
    {siteConfig.bildung && siteConfig.bildung.map(bildung => (
      <article 
        key={bildung.begin.month + bildung.begin.year} 
        className='timeline__item animate-on-scroll'
      >
        <div className="inner">
          <span className="timeline__date">
            <span className="timeline__month">{bildung.begin.month}</span>
            <span className="timeline__year">{bildung.begin.year}</span>
          </span>
          <div className="timeline__card">
            <h2 className='timeline__card-title'>
              {bildung.company
                ? `${bildung.occupation} an der ${bildung.company}`
                : `${bildung.occupation}`} 
              <br />
              <small className='timeline__card-title--small'>
                ({bildung.duration || 'present'})
              </small>
            </h2>
            {/* <p>{bildung.description}</p> */}
          </div>
        </div>
      </article>
    ))}
...
export default styled(Timeline_Bildung)`
